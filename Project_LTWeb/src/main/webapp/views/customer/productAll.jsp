		...
		<c:forEach items="${pro}" var="pro">
			<div class="col-sm-6 col-lg-4 col-xl-3 mb-0 p-1 BoxProduct shadow">
				<div class="card h-50 text-center">
					...
					<div class="card-body">
						<div>
							<h5> <font color="Blue"  class="text-center mt-3 mb-3">${pro.categoryName }</font></h5>
							<h4> <font color="red" class="c-text-lg c-font-bold">${pro.name }</font></h4>
							...
							<p class="card-text c-text-blue-500 c-font-bold">Người bán:
								${pro.storeName }</p>
							<p class="card-text c-text-red-400-500 c-font-bold">Cập nhật:
								${pro.updatedAt }</p>
						</div>
						<a href="product?id=${pro.id }"
							class="btn button-blue d-block shadow" style="background-color:blue;color:white" >Xem Ngay</a>
					</div>
				</div>
			</div>
		</c:forEach>
