...
public class MyOrderCustomerDaoImpl extends DBConnection implements IMyOrderCustomerDao{
	...
	public List<OrderUserModel> myorder(int userid, int status) {
		String sql = "select distinct(Orders.id), Store.name, Delivery.name, Orders.address, Orders.phone, Orders.status ,orders.total_price, Orders.createdAt \r\n"
		...
		try {
			...
			while (rs.next()) {
				...
				order.setStatus(Integer.parseInt(rs.getString(6)));
